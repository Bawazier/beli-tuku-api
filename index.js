...
app.post('/items', (req, res) => {
	...
	if (name && price && description) {
		db.query(`INSERT INTO items(name, price, description) VALUES ('${name}', ${price}, '${description}')`, (err, _results, _fields) => {
			if (!err) {
				res.status(201).send({
					success: true,
...
app.patch('/items/:id', (req, res) => {
	...
	let { id } = req.params;
	if (name || price || description) {
		const set = Object.entries(req.body).map((item)=>{
...
app.delete('/items/:id', (req, res) => {
	let {id} = req.params;
	db.query(`DELETE FROM items WHERE id = ${id}`, (err, respone, _fields) => {
		if(!err){
			res.send({
				success: true,
...
app.get('/items/:id', (req, res) => {
	let {id} = req.params;
	db.query(`SELECT * FROM items WHERE id = ${id}`, (err, respone, _fields) => {
		if(!err){
			res.send({
				success: true,
